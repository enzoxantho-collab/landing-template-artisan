    ...
    <section id="zones" className="bg-slate-50 py-16 md:py-24 scroll-mt-20 lg:scroll-mt-24">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
          {CONFIG.zones.map((z, i) => (
            ...
            >
              ...
              <h3 className="font-black text-[var(--color-primary)] text-xl mb-0.5">{z.city}</h3>
              <p className="text-[var(--color-accent)] text-xs font-bold mb-3">{z.dep}</p>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-6 sm:p-8">
          <h3 className="text-[var(--color-primary)] font-black text-xl mb-4">{CONFIG.seoBlock.title}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-sm text-slate-600 leading-relaxed">
            {CONFIG.seoBlock.items.map((item, i) => (
              <div key={i}>
                <strong className="text-[var(--color-primary)] block mb-1">{item.title}</strong>
                {item.text}
              </div>
            ))}
          </div>
          <div className="mt-6 flex flex-col sm:flex-row gap-3 justify-center sm:justify-start">
            <a
              href={`tel:${PHONE}`}
              className="inline-flex items-center justify-center gap-2 bg-[var(--color-primary)] hover:bg-[var(--color-primary)] text-white font-bold px-6 py-3 rounded-full text-sm transition-all hover:scale-105"
            >
